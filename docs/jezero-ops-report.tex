...
Latency & 15--80\,s & 200--300\,ms & microseconds \\
...
Measured on the recorded run: \textbf{18 Claude calls, 13 of which consulted jev --- 13 consultations, 33 typed questions, 222--361\,ms each}. Against that, the rover itself made 96 jev requests while driving (38 Navcam, 51 Hazcam, plus AEGIS, fault triage and verification). So about 12\,\% of all System One calls happen inside System Two's reasoning, and a consultation adds under half a second to a call that takes 15--95\,s: jev answers in the time a token takes. The other direction is the ledger: at every uplink Claude is told what jev decided while driving --- frames judged, obstacles and keep-outs added, vetoes, reads below the confidence floor, the ground classes seen --- so the slow mind plans on what the fast mind found.
